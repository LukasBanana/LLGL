...
std::string vertexShaderCode = ReadFileContext("vertex.glsl");
std::string fragmentShaderCode = ReadFileContext("fragment.glsl");
\end{lstlisting}
...
Before we continue with the C++ code, we first take a look at the shaders:
\begin{lstlisting}[title={\texttt{vertex.glsl}}]
// GLSL shader version 1.30 (for OpenGL 3.1)
#version 130

// 4x4 projection matrix as single uniform
...
\end{lstlisting}
This simple vertex shader only transforms the vertex position by the current projection matrix
and passes the vertex color (with default interpolation) to the fragment shader.
In this example we use a single uniform to pass the vertex parameters (here ``\texttt{projection}'' only),
but there is also support for uniform buffer objects (in \LLGL they are called constant buffers).
