...
\section{Custom Window Class}
\label{sec:custom_window}

In this example a simple custom implementation of the \texttt{Window} interface is demonstrated,
...
We start the include files of GLFW and LLGL:
\begin{lstlisting}
...
\end{lstlisting}
Now we declare our custom window class and override all necessary interface functions.
We will keep a few secondary functions empty for simplicity:
\begin{lstlisting}
class CustomWindow : public LLGL::Window
{
public:
	CustomWindow(const LLGL::Size& size, const std::string& title);
	
	void SetPosition(const LLGL::Point& position) override;
	LLGL::Point GetPosition() const override;
	
	void SetSize(const LLGL::Size& size, bool useClientArea = true) override;
	LLGL::Size GetSize(bool useClientArea = true) const override;
	...
	LLGL::WindowDescriptor QueryDesc() const override;
	void SetDesc(const LLGL::WindowDescriptor& desc) override;
...
It looks like a lot of work for a small example but the most functions are straight forward.
We start implementing the constructor and the \texttt{CreateGLFWWindow} function:
\begin{lstlisting}
CustomWindow::CustomWindow(const LLGL::Size& size, const std::string& title) :
	size_ ( size               ),
	title_( title              ),
...
GLFWwindow* CustomWindow::CreateGLFWWindow()
{
	// Create GLFW window and stored parameters
	wnd_ = glfwCreateWindow(size_.x, size_.y, title_.c_str(), nullptr, nullptr);
	if (!wnd_)
		throw std::runtime_error("failed to create GLFW window");
}
\end{lstlisting}
The next functions we implement are very simple and used to set and get the position, size, visibility, and title.
They are actually not required for a functioning custom window class, but we also implement them for demonstration purposes:
\begin{lstlisting}
void CustomWindow::SetPosition(const LLGL::Point& position)
{
	// Set GLFW window position
	glfwSetWindowPos(wnd_, position.x, position.y);
}

LLGL::Point CustomWindow::GetPosition() const
{
	// Get position from GLFW window
	LLGL::Point pos;
	glfwGetWindowPos(wnd_, &pos.x, &pos.y);
	return pos;
}

void CustomWindow::SetSize(const LLGL::Size& size, bool useClientArea)
{
	// Set GLFW window size
	glfwSetWindowSize(wnd_, size.x, size.y);
}

LLGL::Size CustomWindow::GetSize(bool useClientArea) const
{
	// Get size from GLFW window
	LLGL::Size size;
	glfwGetWindowSize(wnd_, &size.x, &size.y);
	return size;
}

void CustomWindow::Show(bool show)
{
	// Show or hide GLFW window
	if (show)
		glfwShowWindow(wnd_);
	else
		glfwHideWindow(wnd_);
...
bool CustomWindow::IsShown() const
{
	// Check if GLFW window is visible
	return (glfwGetWindowAttrib(wnd_, GLFW_VISIBLE) != 0);
}

void CustomWindow::SetTitle(const std::wstring& title)
{
	// Convert title form UTF16 to UTF8, and set GLFW window title with "glfwSetWindowTitle"
	title_ = std::string(title.begin(), title.end());
	glfwSetWindowTitle(wnd_, title_.c_str());
}

std::wstring CustomWindow::GetTitle() const
{
	// Return stored title because there is no "glfwGetWindowTitle", and convert from UTF8 to UTF16
	return std::wstring(title_.begin(), title_.end());
}
\end{lstlisting}
We keep the following functions empty for simplicity since they are rarely used:
\begin{lstlisting}
LLGL::WindowDescriptor CustomWindow::QueryDesc() const
{
	// Determine or restore window descriptor ...
	return {};
}

void CustomWindow::SetDesc(const LLGL::WindowDescriptor& desc)
{
	// Recreate GLFW window with new window descriptor ...
}
\end{lstlisting}
Now we cross over to the major functions in the Window interface. The first one is \texttt{Recreate}.
It is mainly used by the OpenGL renderer when a multi-sampled render context is created.
In our custom class we merely destroy the GLFW window and create a new one:
\begin{lstlisting}
void CustomWindow::Recreate(const LLGL::WindowDescriptor& desc)
{
...
On MacOS its is from the type \texttt{NSWindow*} from the Cocoa API.
In this example we only implement it for Win32:
\begin{lstlisting}
void CustomWindow::GetNativeHandle(void* nativeHandle) const
{
...
\end{lstlisting}
The last function we have to implement is the private function \texttt{OnProcessEvents} which is called
by the non-virtual interface function \texttt{Window::ProcessEvents} after all event listeners
have received the same event. Here we poll all window events from GLFW:
\begin{lstlisting}
void CustomWindow::GetNativeHandle(void* nativeHandle) const
{
	...
	// Check if user pressed the close button
...
auto window = std::make_shared<CustomWindow>(LLGL::Size(640, 480), L"LLGL test with GLFW");
...
LLGL::RenderContext* context = renderer->CreateRenderContext(contextDesc, window);
