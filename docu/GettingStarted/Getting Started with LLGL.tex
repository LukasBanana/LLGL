...
\end{lstlisting}
	...
	This layer performs extensive state-, parameter-, and descriptor validation before each draw call.
...
Whenever you load a new renderer, there must not remain any references to this shared object,
...
without disturbing the user with awkward error messages and program crashes.
...
This swap-chain will create its own surface, but we could also specify our own one.
...
// Specify the vertex input attributes. This is obviously only necessary for the vertex shader.
