...
\begin{itemize}
	...
	\item \textbf{Vulkan} \\
	Vulkan renderer is only in an experimental state (\textasciitilde 5\% complete).
\end{itemize}
...
The \href{https://github.com/LukasBanana/GaussianLib}{\textsc{GaussianLib}} is an optional header-only library which is only used in the tests and tutorials.
It provides classes and functions for basic linear algebra with vectors and matrices.
You can omit it if you only want to build the library and the renderer backends.
