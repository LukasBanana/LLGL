...
int main(int argc, char* argv[])
{
    ...
    // Create vertex, geometry (if supported), and fragment shaders
    ...
    if (!logicOpSupported)
        LLGL::Log::Printf("Note: logic pixel operations are not supported by selected renderer");

    LLGL::GraphicsPipelineDescriptor pipelineDesc;
    {
        pipelineDesc.inputVertexAttribs             = LLGL::Parse("rg32f(position),rgb32f(color)");
        ...
        commands->Begin();
        {
            ...
            if (window1.IsShown())
            {
                commands->BeginRenderPass(*swapChain1);
                {
                    ...
                    commands->SetPipelineState(*pipeline[0]);
            ...
            if (window2.IsShown())
            {
                commands->BeginRenderPass(*swapChain2);
                {
                    ...
                    commands->SetPipelineState(*pipeline[1]);
                    commands->SetViewports(2, viewports);
                    commands->SetVertexBuffer(*vertexBuffer);
