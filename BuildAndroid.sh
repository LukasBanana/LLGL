...
generate_app_project()
{
    ...
    if [ -f "$PROJECT_SOURCE_DIR/Android.openxr" ]; then
        ...
        cp "$PLATFORM_SOURCE_DIR/app/src/main/AndroidManifest.xr.xml" "$APP_ROOT/app/src/main/AndroidManifest.xml"
    fi
    # Always remove the XR manifest variant from the copied tree - it's a template, not a build artifact.
    rm -f "$APP_ROOT/app/src/main/AndroidManifest.xr.xml"
