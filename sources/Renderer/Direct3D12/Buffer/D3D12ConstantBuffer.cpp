...
D3D12ConstantBuffer::D3D12ConstantBuffer(ID3D12Device* device)
{
...
void D3D12ConstantBuffer::CreateResourceAndPutView(ID3D12Device* device, UINT bufferSize)
{
    ...
    hwBuffer.CreateResource(device, bufferSize, D3D12_HEAP_TYPE_UPLOAD, D3D12_RESOURCE_STATE_GENERIC_READ);

    /* Create constant buffer view (CBV) */
    D3D12_CONSTANT_BUFFER_VIEW_DESC viewDesc;
    {
        viewDesc.BufferLocation = hwBuffer.Get()->GetGPUVirtualAddress();
