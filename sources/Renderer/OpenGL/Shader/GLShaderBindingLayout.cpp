...
void GLShaderBindingLayout::BuildUniformBindings(const GLPipelineLayout& pipelineLayout)
{
    ...
    {
        const STL::string& name = pipelineLayout.GetCombinedSamplerNames()[i];
        if (!name.empty())
...
void GLShaderBindingLayout::AppendUniformBinding(const STL::string& name, std::uint32_t slot, std::uint32_t size)
{
    bindings_.push_back({ name, slot, size });
    ++numUniformBindings_;
}

void GLShaderBindingLayout::AppendUniformBlockBinding(const STL::string& name, std::uint32_t slot)
{
    bindings_.push_back({ name, slot, 1u });
    ++numUniformBlockBindings_;
}

void GLShaderBindingLayout::AppendShaderStorageBinding(const STL::string& name, std::uint32_t slot)
{
    bindings_.push_back({ name, slot, 1u });
