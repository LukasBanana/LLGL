...
class GLPipelineState : public PipelineState
{
    ...
    public:
        ...
        inline const STL::vector<GLUniformLocation>& GetUniformMap() const
        {
            return uniformMap_;
        }
    ...
    private:
        ...
        using GLNameToUniformMap = std::unordered_map<STL::string, GLActiveUniform>;

    private:

        // Builds the index-to-uniform map.
        void BuildUniformMap(GLShader::Permutation permutation, const STL::vector<UniformDescriptor>& uniforms);
    ...
    private:
        ...
        STL::vector<GLUniformLocation>       uniformMap_;
        Report                          report_;
