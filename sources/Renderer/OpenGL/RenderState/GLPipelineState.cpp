...
void GLPipelineState::BuildUniformMap(GLShader::Permutation permutation, const STL::vector<UniformDescriptor>& uniforms)
{
...
*/
static STL::string GetActiveUniformNameAsIdent(const GLchar* uniformName)
{
    STL::string ident{ uniformName };
    const std::size_t subscriptStartPos = ident.find('[');
    if (subscriptStartPos != STL::string::npos)
...
void GLPipelineState::BuildNameToActiveUniformMap(GLuint program, GLNameToUniformMap& outNameToUniformMap)
{
    ...
    for_range(i, static_cast<GLuint>(numActiveUniforms))
    {
        ...
        const STL::string uniformIdent = GetActiveUniformNameAsIdent(uniformName.data());
