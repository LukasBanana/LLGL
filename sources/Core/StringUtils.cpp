...
static STL::vector<char> ReadFileBufferPrimary(const char* filename)
{
    STL::vector<char> content;
...
{
    const STL::vector<char> content = ReadFileBufferPrimary(filename);
    return STL::string(content.begin(), content.end());
}
...
LLGL_EXPORT STL::vector<char> ReadFileBuffer(const char* filename)
{
    ...
    if (file.good())
    {
        ...
        STL::vector<char> buffer(fileSize);
...
static STL::wstring ToWideStringPrimary(const char* str, std::size_t len)
{
    STL::wstring wstr;
    wstr.resize(len);
    for_range(i, len)
...
void StringPrintf(STL::string& str, const char* format, va_list args1, va_list args2)
{
    ...
    if (len > 0)
    {
        ...
        so it's safe to let ::vsnprintf override STL::string from [0, size()] inclusive.
        */
