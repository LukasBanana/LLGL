...
bool OpenXRSystem::Startup(const XRSystemDescriptor& desc, Report* report)
{
    ...
    if (desc.androidContext.applicationVM == nullptr || desc.androidContext.applicationActivity == nullptr)
    {
        outReport->Errorf("XRRenderSystem::Load failed on Android: XRSystemDescriptor::androidContext requires applicationVM and applicationActivity\n");
        return false;
    }
    androidContext_ = desc.androidContext;
...
bool OpenXRSystem::CreateInstance(const XRSystemDescriptor& desc, Report* report)
{
    ...
    std::snprintf(info.applicationInfo.engineName, XR_MAX_ENGINE_NAME_SIZE, "%s", desc.application.engineName);
    info.applicationInfo.applicationVersion = desc.application.applicationVersion;
    info.applicationInfo.engineVersion      = desc.application.engineVersion;
...
bool OpenXRSystem::QueryViewConfigurations(Report* report)
{
    ...
    {
        viewConfigViews_[i].recommendedImageExtent.width    = xrViews[i].recommendedImageRectWidth;
        viewConfigViews_[i].recommendedImageExtent.height   = xrViews[i].recommendedImageRectHeight;
        viewConfigViews_[i].maxImageExtent.width            = xrViews[i].maxImageRectWidth;
        viewConfigViews_[i].maxImageExtent.height           = xrViews[i].maxImageRectHeight;
...
RenderSystemPtr OpenXRSystem::CreateRenderSystem(const RenderSystemDescriptor& renderSystemDesc, Report* report)
{
    Report* outReport = report != nullptr ? report : &report_;
    ...
    {
        outReport->Errorf(
            "XRRenderSystem::CreateRenderSystem failed: no XR graphics binding registered for renderer '%s'\n",
            renderSystemDesc.moduleName.c_str());
        return nullptr;
    }
    ...
    An explicitly-set value wins.  RenderSystemDescriptor::androidApp is NOT forwarded: it heads
    the glue event loop that only LLGL's own windowing uses, and an XR application draws into
    swap chains owned by the XR runtime rather than a Canvas.
    */
    if (effectiveDesc.androidContext.applicationVM == nullptr)
        effectiveDesc.androidContext.applicationVM = androidContext_.applicationVM;
    if (effectiveDesc.androidContext.applicationActivity == nullptr)
        effectiveDesc.androidContext.applicationActivity = androidContext_.applicationActivity;
    if (effectiveDesc.androidContext.assetManager == nullptr)
        effectiveDesc.androidContext.assetManager = androidContext_.assetManager;
#endif
...
XRSession* OpenXRSystem::CreateSession(const XRSessionDescriptor& sessionDesc, RenderSystem& renderSystem)
{
    ...
    {
        report_.Errorf("XRRenderSystem::CreateSession failed: no active graphics binding (call CreateRenderSystem first)\n");
        return nullptr;
    }
    ...
    {
        report_.Errorf("XRRenderSystem::CreateSession failed: graphics binding refused to produce a session binding chain\n");
        return nullptr;
    }
