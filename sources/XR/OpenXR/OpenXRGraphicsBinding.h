...
\remarks The texture is owned by the OpenXRSwapChain (created via the underlying LLGL::RenderSystem) and is released
...
class GraphicsBinding
{
    ...
    public:
        ...
        */
        ...
        /**
        \brief Hook called immediately before \c xrReleaseSwapchainImage to guarantee that all
        rendering commands targeting the swap-chain image have reached the GPU.
        \remarks Default implementation is a no-op. Backends with explicit submission/synchronization
        (D3D12, Vulkan) don't need this. The D3D11 backend overrides it to \c Flush() the immediate
        context — without that, the runtime's compositor samples stale image contents because D3D11
        commands sit in the user-mode context queue until something forces a kickoff.
        */
        virtual void FlushPendingGpuWork(RenderSystem& /*renderSystem*/) {}
