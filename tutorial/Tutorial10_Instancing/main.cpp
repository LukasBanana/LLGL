...
private:
    ...
    void OnDrawFrame() override
    {
        ...
        {
            commandsExt->SetTexture(*arrayTexture, 1, LLGL::StageFlags::FragmentStage);
            commandsExt->SetConstantBuffer(*constantBuffer, 3, LLGL::StageFlags::VertexStage);

            // Draw all plant instances (vertices: 4, first vertex: 0, instances: numPlantInstances)
            commandsExt->SetSampler(*samplers[0], 2, LLGL::StageFlags::FragmentStage);
            commands->DrawInstanced(4, 0, numPlantInstances);

            #if 1//#ifndef __APPLE__
            // Draw grass plane (vertices: 4, first vertex: 4, instances: 1, instance offset: numPlantInstances)
            commandsExt->SetSampler(*samplers[1], 2, LLGL::StageFlags::FragmentStage);
            commands->DrawInstanced(4, 4, 1, numPlantInstances);
            #endif
        }
